...
from augly.image import blur, color_jitter, encoding_quality, grayscale, hflip, opacity, perspective_transform, pixelization
...
# 	x = crop(x)
# 	return resize(x)
# def change_aspect_ratio_func(x):
# 	x = change_aspect_ratio(x)
# 	return resize(x)

# Define all possible Augmentation Operators
blurOpr = AugOperator("blur", blur, 1.0, AugParam(
									Param("radius", 0.01, ContRange(0.01, 5.0))))
...
# cropOpr = AugOperator( "crop", crop_func, 1.0, AugParam(
...
perspectiveOpr = AugOperator("perspective_transform", perspective_transform, 1.0, AugParam(
							Param("sigma", 1.0, ContRange(0.1, 20.0))))

pixelizationOpr = AugOperator("pixelization", pixelization, 1.0, AugParam(
							Param("ratio", 1.0, ContRange(0.99, 0.2))))
# randomnoise(0.001,0.01)
# rotate(loopcont(0,360))
# shufflepixel(0.001,0.1)
