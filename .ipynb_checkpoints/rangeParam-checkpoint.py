...
class IdentityRange(AbstractRangeParam):
    """
    @desc
    >>> IdentityRange doesn't update the range, it takes in tuple value
    """
    ...
    def __init__(self, 
        ...
        self.__current_value :int = (self.__range_min, self.__range_min)

    def __repr__(self):
        var = """{cyan}IdentityRange({green}
    ...
    def update(self, 
        ...
        self.__current_value = (self.__range_min, self.__range_min)
...
class TupleDecContRange(AbstractRangeParam):
    """
    @desc
    ...
    def __init__(self, 
        ...
        self.__range_min: int = range_min
        self.__range_max: int = range_max
        self.__current_value :int = (self.__range_max, self.__range_max)

    def __repr__(self):
        var = """{cyan}TupleDecContRange({green}
                        range_min={magenta}{range_min}{green},
                        range_max={magenta}{range_max}{green},
                        current_value={magenta}{current_value}{cyan}){reset}"""
        return var.format(
    ...
    def update(self, 
        ...
        if self.__current_value[0]<self.__range_max: # reset
            self.__current_value=(self.__range_min, self.__range_min)
        else:
            val = util.divide_float(
                self.__range_min-self.__range_max,util.aug_app(current_epoch))
            self.__current_value = tuple(p-q for p, q in zip(self.__current_value, (val, val)))
...
class TupleContRange(AbstractRangeParam):
    """
    @desc
    >>> Discrete Range class to encapsulate the range which consists of discrete values
    """
...
class DiscreteRange(AbstractRangeParam):
    ...
    def update(self, 
        ...
        if self.__current_value>self.__range_max: # reset
            ...
            self.__current_value += util.divide_int(
                self.__range_max-self.__range_min,util.aug_app(current_epoch))
...
class DecDiscreteRange(AbstractRangeParam):
    """
    @desc
    >>> Discrete Range class to encapsulate the range which consists of discrete values
    """
    ...
    def __repr__(self):
        var = """{cyan}DecDiscreteRange({green}
                        range_min={magenta}{range_min}{green},
                        range_max={magenta}{range_max}{green},
                        current_value={magenta}{current_value}{cyan}){reset}"""
        return var.format(
